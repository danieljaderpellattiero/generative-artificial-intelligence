...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = self.mu_layer(latent_repr)
        logvar = self.logvar_layer(latent_repr)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ############################################################################################
        # Replace "pass" statement with your code
        latent_repr = self.encoder(torch.cat((x.view(x.size(0), -1), c), dim=1))
        mu = self.mu_layer(latent_repr)
        logvar = self.logvar_layer(latent_repr)
        z = reparametrize(mu, logvar)
        x_hat = (self.decoder(torch.cat((z, c), dim=1))).view(x.size())
        ############################################################################################
        #                                      END OF YOUR CODE                                    #
        ############################################################################################
        return x_hat, mu, logvar
